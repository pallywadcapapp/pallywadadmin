...
(function ($, window, document, undefined) {
    ...
    $.extend(simpleCart.prototype, {
        ...
        _setupCart: function () {
            //this.cart_ele.addClass("container cart-grid panel panel-defaults");
            this.cart_ele.append("<div class='panel-heading cart-heading'><div class='total-cart-count'>Your Commodity 0 items</div><div class='spacer'></div><i class='fa fa-naira-sign total-cart-cost'>0</i><div></div></div>")
            ...
            this.cart_ele.append("<div class='cart-summary-container'>\n\
                                <div class='cart-offer'></div>\n\
                                        <div class='cart-total-amount'>\n\
                                            ...
                                            <div><i class='fa fa-naira-sign total-cart-cost'>0</i></div>\n\
                                            </div>\n\
                                            <div class='cart-checkout'>\n\
                                            <form action='#'>\n\
                                                <button hidden type='button' onclick='clearAll()' class='btn btn-primary'>Clear All</button>\n\
                                            </form>\n\
                                        </div>\n\
                                 </div>");
        },
        ...
        _setEvents: function () {
            ...
            $(this.options.addtoCartClass).on("click", function (e) {
                ...
                mi._addItemToCart(name, cost, 1, id);
        ...
        /* Helper Functions */
        _addItemToCart: function (name, price, count, id) {
        ...
        _displayCart: function () {
            ...
            if (cartArray.length <= 0) {
                output = "<h5>Your cart is empty</h5>";
            }
            for (var i in cartArray) {
                output += "<div class='cart-each-product'>\n\
                       <div class='name'>" + cartArray[i].name + "</div>\n\
                       <div class='quantityContainer'>\n\
                            <input type='number' class='quantity item-count' data-name='" + cartArray[i].name + "' data-price='" +
                    cartArray[i].price +
                    "' min='0' value=" + cartArray[i].count + " name='number'>\n\
                       </div>\n\
                       <div class='quantity-am'><i class='fa fa-naira-sign'>" + cartArray[i].price + "</i></div>\n\
                       </div>";
            }
            return output;
        },
